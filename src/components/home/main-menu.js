...
function MainMenu(){
    ...
    const [mainSubSubCategories, setMainSubSubCategories] = useState([]);
    const [mainBrands, setMenuBrands] = useState([]);
    
    useEffect(() => {
        axios.get('https://backoffice.aodour.tk/api/menu').then((res) => {
            setMainCategories(res.data.data.categories);
            setMainSubCategories(res.data.data.sub_categories);
            setMainSubSubCategories(res.data.data.sub_sub_categories);
            setMenuBrands(res.data.data.brands);
        });
    }, []);
                                    ...
                                    return (
                                        <li key={mainCategories.id}>
                                            <div className='left-cat'>
                                                <a href='#'>{mainCategory.name}<i className="fa fa-angle-right"></i></a>
                                            </div>
                                            { mainSubCategories.length > 0 &&
                                                <div className='sub-dropdown'>
                                                    { mainSubCategories.map((mainSubCategory) => {
                                                        return(
                                                            <div className='inner-wrap' key={mainSubCategory.id}>
                                                                <a href='#'>
                                                                    <strong>{mainSubCategory.name}</strong>
                                                                    <span className="span-plus">+</span>
                                                                    <span className="span-minus">-</span>
                                                                </a>
                                                                { mainSubSubCategories.length > 0 &&
                                                                    <ul>
                                                                        { mainSubSubCategories.map((mainSubSubCategory) => {
                                                                            return(
                                                                                <li key={mainSubSubCategory.id}>
                                                                                    <a href='#'>{mainSubSubCategory.name}</a>
                                                                                </li>
                                                                            )
                                                                        }
                                                                        )}
                                        ...
                                        <div className='inner-wrap'>
                                            <ul>
                                                <div className="scrollcolumn">
                                                    ...
                                                    <li><a id="Element-C" href="#">CIBIO'2 (1) </a></li> 
                                                    <span>C</span> 
